app.controller('DialCtrl', function($scope) {
	...
	$scope.increase = function(i) {
		if ($scope.np[i] < $scope.max) $scope.np[i]++;
	}
	
	$scope.decrease = function(i) {
		if ($scope.np[i] > $scope.min) $scope.np[i]--;
	}	
	
	$scope.getAnswer = function() {
		return $scope.np.join("");
	}
});
		...
		this.each(function(){
			...
			element.update = function(){
				...
				return this;
                        ...
                        np.children(".dp-numberPicker-add").click(function(){element.increase();});
                        np.children(".dp-numberPicker-sub").click(function(){element.decrease();});
                        np.children(".dp-numberPicker-input").change(function(){element.setValue(betterParseFloat( np.children(".dp-numberPicker-input").val()));});
...
//			np.children(".dp-numberPicker-input").on("change", function(){element.setValue(betterParseFloat( np.children(".dp-numberPicker-input").val()));});
        ...
        var overlayDiv = function(choice){
            ...
            var answer = 130;
            //var valp = $("#np1").options.value;
            var valnp = 100 * document.getElementById("np1").options.value + 10 * document.getElementById("np2").options.value + document.getElementById("np3").options.value;
            if (valnp == answer) {
               correct = "#correct";
